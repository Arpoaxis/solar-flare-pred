"""
Train the time-pooled CNN baseline with early stopping.

This script trains a CNN baseline that processes all four timesteps in a
sample and pools the per-timestep representations with temporal mean pooling.
It uses the saved AR-disjoint train/validation splits, logs epoch losses to
CSV, and saves both latest and best checkpoints.
...
# Output naming.
# Example alternative:
# RUN_NAME = "cnn_timepool"
RUN_NAME = "cnn_timepool_es"
...
def run_one_epoch(
    ...
    for step, (x, y) in enumerate(dataloader):
        # Full sample shape is [B, 4, 10, 256, 256]. The time-pooling model
        # uses all four timesteps directly.
...
def main() -> None:
    """
    Train the time-pooled CNN baseline and apply early stopping.
    """
    ...
    model = CNNTimeMeanPool(in_channels=10).to(device)
    ...
    with open(log_path, "w", newline="") as log_file:
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            stop, best_val, bad_epochs, is_best = should_stop_early(
                val_loss=val_loss,
                best_val=best_val,
